...
class ReadyToLocalize(object):
    ...
    def printPublishConfigurationResult(self):
        ...
        for i in range(list_size[0]):
            ...
            ts=time.time()
            print("%s, ANCHOR,0x%0.4x, %s" % (ts, device_list[i], str(anchor_coordinates)))
            if self.osc_udp_client is not None:
                self.osc_udp_client.send_message(
    ...
    def printPublishAnchorConfiguration(self):
        ...
        for anchor in self.anchors:
            ts=time.time()
            print("%s, ANCHOR,0x%0.4x,%s" % (ts,anchor.network_id, str(anchor.coordinates)))
            if self.osc_udp_client is not None:
                self.osc_udp_client.send_message(
...
if __name__ == "__main__":
    ...
    anchors = [DeviceCoordinates(0x6129, 1, Coordinates(0, 1669, 1016)),
               DeviceCoordinates(0x604f, 1, Coordinates(3024, 5886, 1535)),
               DeviceCoordinates(0x6020, 1, Coordinates(3545, 0, 2595)),
