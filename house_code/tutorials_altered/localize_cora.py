...
from pypozyx.definitions.registers import (POZYX_POS_ERR_X)
...
class ReadyToLocalize(object):
    ...
    def loop(self, elapsed, timeDifference, pos_error, index=None):
        """Performs positioning and displays/exports the results."""
        position = Coordinates()
        status = self.pozyx.doPositioning(
            position, self.dimension, self.height, self.algorithm, remote_id=self.remote_id)
        if status == POZYX_SUCCESS:
            return self.printPublishPosition(position, elapsed, timeDifference, pos_error)
        else:
            self.printPublishErrorCode("positioning")

        return "unexpected error"

    def printPublishPosition(self, position, elapsed, timeDifference, pos_error):
        ...
        print(output)
        lib.getPositionError(self, pos_error, remote_id=None)


        return output, lib.getPositionError(POZYX_POS_ERR_X, pos_error, remote_id)
...
if  __name__ == "__main__":
    ...
    remote_id = 0x605a                 # remote device network ID
    ...
    try:
        while True:
            ...
            singleLineOutput = r.loop(elapsed, timeDifference, lib)    # the iterate_file method of r prints data to the console and returns what is printed
            if to_use_file:
